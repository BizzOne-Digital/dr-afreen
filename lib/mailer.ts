...
export async function sendBookingEmail(data: BookingEmailData): Promise<void> {
  const adminEmail = process.env.ADMIN_EMAIL || "moonhomeopathic@gmail.com";
  ...
  const htmlContent = `
    ...
    <body>
      <div class="container">
        ...
        <div class="footer">
          Moon Homeopathy — Toronto, Canada &nbsp;|&nbsp; Reply promptly on WhatsApp: +1 647 781 9461
        </div>
      </div>
    </body>
    </html>
  ...
  await transporter.sendMail({
    from: `"Moon Homeopathy Website" <${process.env.SMTP_USER}>`,
    to: adminEmail,
...
export async function sendContactEmail(data: {
  ...
  await transporter.sendMail({
    ...
    html: `
      <div style="font-family: Georgia, serif; padding: 24px; background: #FFF8EF;">
